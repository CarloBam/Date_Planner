...
def render_receiver_view(token):
    ...
    with col2:
        ...
        # Fun Outfit Guide based on Weather
        is_rainy = plan.get('is_rainy', False)
        is_windy = plan.get('is_windy', False)

        st.markdown("### 👗 Weather & What to Wear")
        if is_rainy:
            st.write("🌦️ **Forecast:** Looks like it might rain! You might want to grab a cute raincoat, umbrella, or warm layer.")
        elif is_windy:
            st.write("🌬️ **Forecast:** The Cape Town wind is blowing! Consider wearing something that won't fly away, and tie your hair back if you want!")
        else:
            st.write("☀️ **Forecast:** Pretty clear skies! Dress comfortably for a lovely day.")
        
        st.caption("_PS: This outfit recommendation was automatically generated by the DateCart app based on the weather forecast... not by your date!_ 🤐")
        ...
        if plan["status"] == "pending":
            ...
            if st.button("Sorry, I can't make it (No)", type="secondary"):
                try:
